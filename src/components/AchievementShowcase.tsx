...
const fetcher = (url: string) => 
  fetch(url)
    .then(res => {
      if (!res.ok) {
        // For 404 errors, we'll just return empty achievements
        if (res.status === 404) {
          return { achievements: [] };
        }
...
export default function AchievementShowcase({ userId, className = '' }: AchievementShowcaseProps) {
  ...
  // Use SWR for real-time data fetching with auto-revalidation
  ...
  // When achievements are loading, show a loading skeleton
  if (isLoading) {
    return (
  ...
  // If we have a non-404 error, show the error message
  ...
  // If no achievements or 404, show a friendly message
  ...
  // If we have achievements, display them
  return (
    <div className={`achievement-showcase ${className}`}>
      <h3 className="text-lg font-semibold mb-3">Achievements</h3>
